...
countsTable_raw <- read_delim('Porites_astreoides/Pilot_Fail_New_Concat_Compiled/Counts/STAR_Salmon_Past_Smic_NumReads_matrix_Past.txt')
dim(countsTable_raw)

###read in sample names to change novogene_# to coral id number)
sample_names <- read_excel('Sample_Labels_Novogene_vs_RNA#.xlsx') %>%
...
write.table(countsTable, "Porites_astreoides/Pilot_Fail_New_Concat_Compiled/Dedup_Mote_Past_Counts_full_cleannames.txt")
...
#calculate number of samples with zero reads for a given contig
Numzeros = apply(countsTable, 1, function(x){sum(x==0)})
table(Numzeros>=(0.9*219))
table(Numzeros>=(0.8*219))
table(Numzeros>=(0.7*219))
table(Numzeros>=(0.6*219))
table(Numzeros>=(0.5*219))
table(Numzeros>=(0.4*219))
...
sum(prop.nullv2>50)
sum(prop.nullv2>40)
names(prop.nullv2[prop.nullv2>75])
#calculating minimum replicates per treatment if we remove samples > 75% null counts

names(prop.nullv2[prop.nullv2<75])
conds75<-conds[conds$Sample%in%names(prop.nullv2[prop.nullv2<75]),]
table(conds75$Origin)
table(conds75$Geno)
table(conds75$Temp)
table(conds75$Timepoint)
table(conds75$Origin_Temp_Timepoint)

length(table(conds75$Origin_Temp_Timepoint))
sum(table(conds75$Origin_Temp_Timepoint)>=3)
...
pdf(file="../Mean3NullCountsv375percorless.pdf",14, 14)
...
dds <- DESeqDataSetFromMatrix(countData=countsTable75, colData=conds75, design=~ Origin_Temp_Timepoint)
...
realvstCounts<-assay(vstCounts)

ResresIn_30_T1vIn_30_T2 <- read.delim("resIn_30_T1vIn_30_T2.txt", header=T, sep="\t")
head(ResresIn_30_T1vIn_30_T2)

sum(ResresIn_30_T1vIn_30_T2$padj<0.10 & !(is.na(ResresIn_30_T1vIn_30_T2$padj)) & ResresIn_30_T1vIn_30_T2$log2FoldChange>0)

realvstCounts["GCKDGN101A0607_Past",colnames(realvstCounts)[c(grep("MoteIn_._30_T1",colnames(realvstCounts)),grep("MoteIn_._30_T2",colnames(realvstCounts)))]]

realvstCounts["GCKDGN101A14KT_Past",colnames(realvstCounts)[c(grep("MoteIn_._30_T1",colnames(realvstCounts)),grep("MoteIn_._30_T2",colnames(realvstCounts)))]]

realvstCounts["GCKDGN101A48ZU_Past",colnames(realvstCounts)[c(grep("MoteIn_._30_T1",colnames(realvstCounts)),grep("MoteIn_._30_T2",colnames(realvstCounts)))]]
