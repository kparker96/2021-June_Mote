...
write_xlsx(send_out,"send_out.xlsx")

send_out %>%
  dplyr::count(TP)

send_out_dupe_check <- send_out %>%
 dplyr::count(Sample_ID) %>%
  filter(n > 1)
```
...
TP_geno <- send_out_breakdown %>%
 dplyr::count(TP, geno) 
write_xlsx(TP_geno,"TP_Geno.xlsx")

TP <- send_out_breakdown %>%
 dplyr::count(TP)

write_xlsx(TP,"TP.xlsx")

geno <- send_out_breakdown %>%
  dplyr::count(geno)
write_xlsx(geno,"geno.xlsx")

Temp <- send_out_breakdown %>%
  dplyr::count(Temperature)

write_xlsx(Temp,"Temp.xlsx")

TP_Temp <- send_out_breakdown %>%
  dplyr::count(TP, Temperature)

write_xlsx(TP_Temp,"TP_Temp.xlsx")
...
```{r}
x <- nano %>%
  mutate(Reextract = case_when(origin == "Re-extraction" ~ 1),
         RNALater = case_when(origin == "RNALater" ~ 1),
         FlashFrozen = case_when(origin == "Flash Frozen" ~ 1))

y <- x %>%
  select(Sample_ID, Reextract, RNALater, FlashFrozen) %>%
  group_by(Sample_ID) %>%
  gather(key = origin, value = value, -Sample_ID) %>%
  filter(value == 1)
...
y_dupe_ID <- y %>% 
  filter(Sample_ID %in% y_dupe$Sample_ID) 
