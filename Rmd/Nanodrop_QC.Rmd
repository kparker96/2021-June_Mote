...
dupe_check <- usable_re_x %>%
  dplyr::count(Sample_ID) %>%
  filter(n > 1)

remove_dupe <- usable_re_x %>%
  filter(Sample_ID %in% dupe_check$Sample_ID) %>%
  arrange(Sample_ID) %>%
  filter(origin != "RNALater")

usable_re_x %>%
  filter(Sample_ID != Sample_ID %in% remove_dupe$Sample_ID)


good_re_x <- 
  
  # Usable re_x, but with duplicates removed 
  # criteria 
  #     (1) Is above 18.5 ng/uL 
  #     (2) Is from RNALater if possible 

send_out <- rbind(goodRNA, good_re_x)



still_extract <- lowyield %>%
  filter(!Sample_ID %in% reextract$Sample_ID)

setdiff(lowyield$Sample_ID, reextract$Sample_ID)
