...
Past_PAM_TP <-  PAST %>%
  ...
  select(Genotype = Geno, Treatment, Temp, TP_ID, FvFm) %>%
...
Past <- rbind(Past_T5, Past_T5_PAM)

ggplot(a, aes(x = Temp, y = FvFm)) +
  geom_point() +
  facet_grid(Timepoint~Geno)

ggplot(Past, aes(x = Temp, y = FvFm, color = Timepoint)) +
  geom_point() +
  facet_wrap(~Geno) +
  theme_bw() +
  theme(legend.position = "none") +
...
Court_df$FvFm<- as.numeric(Court_df$FvFm)
