---
title: "DEG_Dedup_vs_OG_Comparison"
output: html_document
date: "2023-05-16"
editor_options: 
  chunk_output_type: console
---
#Comparing DEGs for 1st Pass and Deduplicated Samples #
...
```{r}
#Set WD
setwd("/Users/kpark/OneDrive/Documents/Barshis_Lab/2021-June_Mote/data/Sequencing/Porites_astreoides")
...
# Load Data #
```{r}
# DEG Counts table from first pass 
OG <- read.delim('DESeq/AllDESeqResults/2021-June-Mote_Past_DEcalc.txt') %>%
  dplyr::mutate(Trial = "original")

# DEG Counts table from Deduplicated 
NG <- read.delim('Pilot_Fail_New_Concat_Compiled/DESeq_Contrasts/2023-05-10_dedup_DEGs.tab') %>%
  dplyr::mutate(SitevSite = case_when(grepl("resIn", Contrast) & grepl("vIn", Contrast) ~ "InvIn",
                               grepl("resIn", Contrast) & grepl("vOff", Contrast) ~ "InvOff",
                               grepl("resOff", Contrast) & grepl("vIn", Contrast) ~ "OffvIn",
                               grepl("resOff", Contrast) & grepl("vOff", Contrast) ~ "OffvOff")) %>%
  mutate(Trial = "deduped")

# Combine OG and NG DEG counts 
DEGs <- rbind(OG, NG)

# DFs Separating by Site # 
# Inshore 
InvIn <- DEGs %>%
  filter(SitevSite == "InvIn") %>%
  gather(key = "DEG_type", value = "counts", -Contrast, -SitevSite, -TP1, -TP2, -Temp, -Trial)
...
# Plotting Total DEGs, all Contrasts#
```{r}
# Total DEGs per Timepoint per Sequencing group 
ggplot(DEGs, aes (x = TP2, y = TotalNumDEGs, fill = TP2)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_grid(Trial~Temp) +
  theme(legend.position = "bottom") +
  ggtitle("All Contrasts, Both Sites")

ggplot(InvIn, aes(x = TP2, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_wrap(~Trial) +
  theme(legend.position = "bottom") +
  ggtitle("All Inshore vs Inshore Contrasts")

ggplot(OffvOff, aes(x = TP2, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_wrap(~Trial) +
  theme(legend.position = "bottom") +
  ggtitle("All Offshore vs Offshore Contrasts")

```

#Controls vs Heatstress for Each TP
```{r}
In_C_vs_HS <- InvIn %>%
  filter(TP1 != "T0") %>%
  filter(TP1 == TP2)

ggplot(In_C_vs_HS, aes(x = Temp, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_grid(Trial~TP2) +
  theme(legend.position = "bottom") +
  ylim(0, 11500) +
  ggtitle("Inshore Control vs Heat Stress DEGs")

Off_C_vs_HS <- OffvOff %>%
  filter(TP1 != "T0") %>%
  filter(TP1 == TP2)

ggplot(Off_C_vs_HS, aes(x = Temp, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_grid(Trial~TP2) +
  theme(legend.position = "bottom") +
  ylim(0, 11500) +
  ggtitle("Offshore Control vs Heat Stress DEGs")

deduped_C_vs_HS <- DEGs %>%
  filter(Trial == "deduped") %>%
  filter(SitevSite == "OffvOff" | SitevSite == "InvIn") %>%
  filter(TP1 != "T0") %>%
  filter(TP1 == TP2) %>%
  mutate(site = case_when(SitevSite == "InvIn" ~ "Inshore",
                          SitevSite == "OffvOff" ~ "Offshore")) %>%
   gather(key = "DEG_type", value = "counts", -Contrast, -SitevSite, -TP1, -TP2, -Temp, -Trial, -site) %>%
  filter(DEG_type == "TotalNumDEGs")

write_csv(deduped_C_vs_HS, "DEG_barplot.csv")

deduped_C_vs_HS <- read.csv("DEG_barplot.csv")
deduped_C_vs_HS$Temp <- as.character(deduped_C_vs_HS$Temp)

ggplot(deduped_C_vs_HS, aes(x = TP2, y = counts, fill = site)) +
  ...
  facet_grid(Temp~TP2, scales = "free_x") +
  theme_bw() +
  ylab("# of DEGs") +
  theme(legend.position="bottom",
...
ggsave("DEG_barplot.png", width = 17, height = 10, units = "in", dpi = "retina")
...
deduped_C_vs_HS %>%
  filter(Temp == "34") %>%
  ggplot(aes(x = TP2, y = counts, fill = site)) +
  scale_fill_manual(values=c('#8ADCE3', "#003A9D")) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_bw() +
  theme(legend.position = "bottom",
...
ggplot(In_Controls, aes(x = TP2, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_grid(~Trial) +
  theme(legend.position = "bottom") +
  ggtitle("Inshore Controls")

ggplot(OffvIn, aes(x = TP2, y = counts, fill = DEG_type)) +
  geom_bar(position="dodge", stat= "identity") +
  facet_grid(~Trial) + 
  theme(legend.position = "bottom") +
...
######## ORIGINAL CODE FOR OG COUNTS ############
```{r}
df_DEG_counts <- read.delim("DESeq/AllDESeqResults/2021-June-Mote_Past_DEcalc.txt") 
